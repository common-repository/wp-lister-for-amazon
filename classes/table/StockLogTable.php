...
class WPLA_StockLogTable extends WP_List_Table {
    ...
    function get_columns(){
        $columns = array(
            ...
            'product_id'        => __( 'Product ID', 'wp-lister-for-amazon' ),
            'sku'               => __( 'SKU', 'wp-lister-for-amazon' ),
            'old_stock'         => __( 'Old Stock', 'wp-lister-for-amazon' ),
            'new_stock'         => __( 'New Stock', 'wp-lister-for-amazon' ),
            'caller'            => __( 'Caller', 'wp-lister-for-amazon' ),
            'method'			=> __( 'Method', 'wp-lister-for-amazon' ),
    ...
    function extra_tablenav( $which ) {
        ...
        $date_from = !empty( $_REQUEST['date_from'] ) ? wpla_clean( $_REQUEST['date_from'] ) : '';
        $date_to   = !empty( $_REQUEST['date_to'] ) ? wpla_clean( $_REQUEST['date_to'] ) : '';
        ?>
        <div class="alignleft actions" style="">
            ...
            <?php _e( 'Filter date range:', 'wp-lister-for-ebay' ); ?>
            <input type="text" class="range_datepicker from" id="date_from" name="date_from" placeholder="mm-dd-yyyy" value="<?php echo esc_attr($date_from); ?>" size="8" />
            <input type="text" class="range_datepicker to" id="date_to" name="date_to" placeholder="mm-dd-yyyy" value="<?php echo esc_attr($date_to); ?>" size="8" />
    ...
    function getPageItems( $current_page, $per_page ) {
        ...
        $date_from = !empty( $_REQUEST['date_from'] ) ? esc_sql( wpla_clean( $_REQUEST['date_from'] ) ) : '';
        $date_to   = !empty( $_REQUEST['date_to'] ) ? esc_sql( wpla_clean( $_REQUEST['date_to'] ) ) : '';
        ...
        // search box
        if ( isset( $_REQUEST['s'] ) ) {
            $tables_sql .= " LEFT JOIN {$wpdb->postmeta} pm ON l.product_id = pm.post_id";
        ...
        if ( !empty( $date_to ) && !empty( $date_from ) ) {
            ...
            $dt->setDate( $date[2], $date[0], $date[1] );
            $dt->setTime(0, 0, 0 );
            ...
            $where_sql .= " AND timestamp BETWEEN '$date_from' AND '$date_to' ";
        }
